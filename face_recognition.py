    ...
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in featuers:
                ...
                cursor.execute("select name from dataset where id="+str(id))
                n=cursor.fetchone()
                n='+'.join(n)

                cursor.execute("select type from dataset where id="+str(id))
                r=cursor.fetchone()
                r='+'.join(r)

                cursor.execute("select no from dataset where id="+str(id))
                i=cursor.fetchone()
                i='+'.join(i)
                ...
                if confidence > 77:
                    ...
                    cv2.putText(img,f"Type:{r}",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
                    cv2.putText(img,f"No:{i}",(x,y-30),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
