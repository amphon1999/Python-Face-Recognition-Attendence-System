...
class add:
    ...
    # This part is image labels setting start
        ...
        bg_img.place(x=0, y=130, width=1366, height=768)
         ...
         #title section
        title_lb1 = Label(bg_img,text="Welcome to Data Pannel",font=("verdana",30,"bold"),bg="white",fg="navyblue")
        ...
        # Left Label Frame 
        left_frame = LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Data Details",font=("verdana",12,"bold"),fg="navyblue")
        left_frame.place(x=10,y=10,width=660,height=480)

        #Class  Information
        class_Student_frame = LabelFrame(left_frame,bd=2,bg="white",relief=RIDGE,text="Class Data Information",font=("verdana",12,"bold"),fg="navyblue")
        class_Student_frame.place(x=10,y=50,width=635,height=230)

        # Student No.
        studentId_label = Label(class_Student_frame,text="No:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        studentId_label.grid(row=0,column=0,padx=5,pady=5,sticky=W)

        studentId_entry = ttk.Entry(class_Student_frame,textvariable=self.var_no,width=15,font=("verdana",12,"bold"))
        studentId_entry.grid(row=0,column=1,padx=5,pady=5,sticky=W)

        #Name
        name_label = Label(class_Student_frame,text="Name:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        name_label.grid(row=0,column=2,padx=5,pady=5,sticky=W)

        name_entry = ttk.Entry(class_Student_frame,textvariable=self.var_name,width=15,font=("verdana",12,"bold"))
        name_entry.grid(row=0,column=3,padx=5,pady=5,sticky=W)

        #Type class
        type_label = Label(class_Student_frame,text="Type:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        type_label.grid(row=1,column=0,padx=5,pady=5,sticky=W)

        div_combo=ttk.Combobox(class_Student_frame,textvariable=self.var_type,width=13,font=("verdana",12,"bold"),state="readonly")
        div_combo["values"]=("Studen","Teacher")
        div_combo.current(0)
        div_combo.grid(row=1,column=1,padx=5,pady=5,sticky=W)

        #Gender
        gender_label = Label(class_Student_frame,text="Gender:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        ...
        #Phone Number
        student_mob_label = Label(class_Student_frame,text="Phone:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        student_mob_label.grid(row=1,column=2,padx=5,pady=5,sticky=W)

        student_mob_entry = ttk.Entry(class_Student_frame,textvariable=self.var_phone,width=15,font=("verdana",12,"bold"))
        student_mob_entry.grid(row=1,column=3,padx=5,pady=5,sticky=W)
        ...
        right_frame = LabelFrame(main_frame, bd=2, bg="white", relief=RIDGE,
                                 text="Data Details", font=("verdana", 12, "bold"), fg="navyblue")
        ...
        search_combo["values"] = ("Select", "Type")
        ...
        # create table
        self.student_table = ttk.Treeview(table_frame, column=("ID","No","Name","Type","Gender","Phone","Email","Address","Data","Photo"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        self.student_table.heading("No", text="No")
        ...
        self.student_table.heading("Data", text="Data")
        self.student_table.heading("Photo", text="Photo")
        self.student_table["show"] = "headings"

        # Set Width of Colums   Semester
        self.student_table.column("ID", width=100)
        self.student_table.column("No", width=100)
        ...
        self.student_table.column("Data", width=100)
...
# ==================Function Decleration==============================
    ...
    def add_data(self):
        if self.var_no.get() == "" or self.var_name.get == "" or self.var_type.get() == "" or self.var_gender.get() == "" or self.var_phone.get() == "" or self.var_email.get() == "" or self.var_address.get() == "" or self.var_radio1.get() == "" :
            ...
            try:
                ...
                mycursor.execute("INSERT INTO `dataset`(" +
                 "`no`," +
                 "`name`," +
                 "`type`," +
                 "`gender`," +
    ...
    def update_data(self):
        if self.var_no.get() == "" or self.var_name.get == "" or self.var_type.get() == "" or self.var_gender.get() == "" or self.var_phone.get() == "" or self.var_email.get() == "" or self.var_address.get() == "" or self.var_radio1.get() == "" :
            ...
            try:
                ...
                queryUpdate = "update dataset set name ='" + self.var_name.get() +"',type ='"+self.var_type.get()+"',gender='"+self.var_gender.get()+"',phone='"+self.var_phone.get()+"',eamil='"+self.var_email.get()+"',address='"+self.var_address.get()+"',data='"+self.var_data.get()+"',radio1='"+self.var_radio1.get()+"' where no ='"+self.var_no.get()+"'"
                messagebox.showinfo(
    ...
    def delete_data(self):
        if self.var_no.get() == "":
            messagebox.showerror(
                "Error", " Must be Required!", parent=self.root)
        else:
            try:
                ...
                if delete > 0:
                    conn = mysql.connector.connect(host='localhost', user='root', password='', database='face_recognition')
                    mycursor = conn.cursor()
                    sql = "delete from dataset where no=%s"
                    val = (self.var_no.get(),)
    ...
    def search_data(self):
        if self.var_search.get() == "" or self.var_searchTX.get() == "Select":
            ...
            try:
                ...
                sql = "SELECT `id`, `no`, `name`, `type`, `gender`, `phone`, `email`, `address`,`photo` FROM `dataset` WHERE `id` "  + \
...
# ==================================Generate Data set take image=========================
    ...
    def generate_dataset(self):
        if self.var_no.get() == "" or self.var_name.get == "" or self.var_type.get() == "" or self.var_gender.get() == "" or self.var_phone.get() == "" or self.var_email.get() == "" or self.var_address.get() == "" :
            messagebox.showerror(
                "Error", "Please Fill All Fields are Required!", parent=self.root)
        else:
            try:
                ...
                queryUpdate = "update dataset set name ='" + self.var_name.get() +"',type ='"+self.var_type.get()+"',gender='"+self.var_gender.get()+"',phone='"+self.var_phone.get()+"',eamil='"+self.var_email.get()+"',address='"+self.var_address.get()+"' where no ='"+self.var_no.get()+"'"
